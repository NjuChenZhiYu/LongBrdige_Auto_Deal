...
class _DummyQuoteCtx:
    def get_owner_plate(self, symbols):
        df = pd.DataFrame([{"plate_type": "INDUSTRY", "plate_name": "智能物流"}])
        return 0, df
...
async def main() -> None:
    ...
    futu_client.get_capital_flow = lambda symbol: pd.DataFrame()
    ...
    fmi.calculate_hk_capital_flow = lambda symbol, window: {
        "window_days": window,
        "main_in_flow_hkd": 1000000.0,
        "total_in_flow_hkd": 600000.0,
    ...
    analyst._call_llm_with_retry = _fake_call

    result = await analyst.generate_single_stock_futu_report("02598", enable_grounded_search=True)
    prompt_text = captured["prompt"]

    print("OK=", result.get("ok"))
    print("HAS_SECTION6=", "6. 联网检索证据（固定三行）" in prompt_text)
    print("HAS_DOMAIN_RULE=", "对标来源域名：至少3个" in prompt_text)
    print("HAS_BJ_TIME_RULE=", "检索时间：YYYY-MM-DD HH:MM（北京时间）" in prompt_text)
