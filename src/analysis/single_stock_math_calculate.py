...
def calculate_ema_derivatives(df: pd.DataFrame, current_price: float) -> dict:
    """计算均线衍生指标、当日技术标签及 T-1 成交量 regime。

    价格/趋势侧（实时）：EMA5/EMA20/V5/V20/A5/Bias20 均含盘中 current_price。
    成交量侧（T-1 锚）：volume_regime 只取已收盘完整交易日，不使用盘中数据，
        自带 1 天滞后以换取稳健度（避免盘中前段成交量外推失真）。
    ...
    # --- 成交量 regime（T-1 锚，必须在拼接 current_price 行之前计算）---
    volume_regime = "中性"
    r_prev1d_5d = 1.0
    r_prev1d_20d = 1.0
    r_5d_20d = 1.0
    if "volume" in df.columns:
        closed_vol: pd.Series = pd.to_numeric(df["volume"], errors="coerce").dropna()  # type: ignore[assignment]
        if len(closed_vol) >= 20:
            v_prev1d = float(closed_vol.iloc[-1])
            v_avg5 = float(closed_vol.iloc[-5:].mean())
            v_avg20 = float(closed_vol.iloc[-20:].mean())
            if v_avg5 > 0 and v_avg20 > 0:
                r_prev1d_5d = round(v_prev1d / v_avg5, 3)
                r_prev1d_20d = round(v_prev1d / v_avg20, 3)
                r_5d_20d = round(v_avg5 / v_avg20, 3)
    ...
    _TV_MAP: dict[tuple[str, str], str] = {
        ("【主升浪加速：长短共振】",       "放量"): "【主升浪加速：长短共振】+【放量】✅ 真突破，主升浪开启 → 加仓/追涨",
        ("【主升浪加速：长短共振】",       "缩量"): "【主升浪加速：长短共振】+【缩量】✅ 极致控盘加速，主力锁仓 → 死拿不动",
        ("【主升浪降速：高位震荡/诱多】",  "放量"): "【主升浪降速：高位震荡/诱多】+【放量】❌ 真出货，顶部确认 → 立刻止盈",
        ("【主升浪降速：高位震荡/诱多】",  "缩量"): "【主升浪降速：高位震荡/诱多】+【缩量】⚠️ 强势整理，还有新高 → 持有/回调加仓",
        ("【主跌浪加速：空头长短共振】",   "放量"): "【主跌浪加速：空头长短共振】+【放量】❌ 真出货，下跌刚开始 → 立刻止损",
        ("【主跌浪加速：空头长短共振】",   "缩量"): "【主跌浪加速：空头长短共振】+【缩量】✅ 假破位洗盘，黄金坑 → 越跌越买",
        ("【跌势放缓：左侧建仓观察区】",   "缩量"): "【跌势放缓：左侧建仓观察区】+【缩量】✅ 抛压枯竭，底部确认 → 左侧分批建仓",
        ("【跌势放缓：左侧建仓观察区】",   "放量"): "【跌势放缓：左侧建仓观察区】+【放量】⚠️ 多空分歧，还会震荡 → 观望/等缩量再买",
        ("【短期趋势转多：均线金叉预备】", "放量"): "【短期趋势转多：均线金叉预备】+【放量】✅ 有效转多，资金认可 → 轻仓介入",
        ("【短期趋势转多：均线金叉预备】", "缩量"): "【短期趋势转多：均线金叉预备】+【缩量】⚠️ 弱修复，量能不足 → 观望为主",
        ("【短期趋势转空：均线死叉预备】", "放量"): "【短期趋势转空：均线死叉预备】+【放量】❌ 真回调，还有下跌空间 → 减仓",
        ("【短期趋势转空：均线死叉预备】", "缩量"): "【短期趋势转空：均线死叉预备】+【缩量】✅ 洗盘回调，黄金坑 → 回调加仓",
    }
