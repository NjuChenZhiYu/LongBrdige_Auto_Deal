...
{signal_text}"""
    ...
    2. 基本面与估值透视（中长期推演，150-250字）
        ...
        * 【买方四大公理映射】：必须审视标的业务契合了以下哪几条底层公理，并据此定性资产属性（防御型现金奶牛 vs 进攻型高爆发成长）：1.中外剪刀差（成本RMB化，收益USD/外汇化）；2.AI智力边际成本趋零；3.物理世界运转效率跃升；4.老龄化不可逆（驱动机器换人与颠覆性创新药刚需）。若不符合任何一条公理，直接给出“不予买入”结论；若满足一条或多条，可上调中长期评分。
